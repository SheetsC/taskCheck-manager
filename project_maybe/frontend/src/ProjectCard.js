...
export function ProjectCard({ project, user, tasks, client, _project}) {
  ...
  const userUndoneTasks= tasks?.filter(task=> task.user_id === user.id && task.complete !== true)
  ...
  const userComponents =  users.map(_user =>{
    return (
      <div class='mt-03 cursor-pointer select-none' key={_user.id}>
        <ul>{_user.name} | 
        username:  {_user.username} | 
        Logged in: {_user.logged_in? "True" : "False"} |
  ...
  return (
    ...
    <div className="flex flex-col gap-2 mb-4">
      ...
      <div className='items-center text-center mb-1'>
        ...
        {userUndoneTasks.length > 0 ? " You have tasks to do. " : ""}
        {otherUserTasks.length > 0 ? " (Other users have tasks to do)" : ""}
      </div>
    </div>
  </div>):
  client ? (<div>{_project.name}</div>):(null)
)}
